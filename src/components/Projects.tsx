        ...
        >
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="flex-1">
              ...
              <h3 className="text-3xl md:text-4xl font-black text-[var(--c-text)] mb-2 group-hover:text-[#7000ff] transition-colors">Akademia</h3>
              <p className="text-[var(--c-muted)] font-mono text-sm mb-4">{tx.featured.tagline}</p>
              <p className="text-[var(--c-muted)] leading-relaxed mb-6 max-w-xl">{tx.featured.description}</p>
            ...
            <div className="lg:w-48 flex flex-row lg:flex-col gap-4 lg:justify-start">
              {tx.featured.metrics.map((m) => (
                <div key={m.label} className="flex-1 lg:flex-none p-4 rounded-xl bg-[rgba(112,0,255,0.04)] border border-[rgba(112,0,255,0.08)] text-center lg:text-left">
                  <div className="text-xs text-[var(--c-muted)] font-mono mb-1 uppercase tracking-wider">{m.label}</div>
